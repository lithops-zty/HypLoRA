...
# 7a. Normal case: vocab_sample_k=50 (stratified sample)
...
print(f"[OK] 7a select_groups_via_llm (sampled): {len(llm_groups)} groups, "
      f"tokens covered: {sum(len(v) for v in llm_groups.values())}")

# 7b. vocab_sample_k=0 means full vocabulary (no sampling)
with _mock.patch.object(_ea, "LMCompletion", MockLMClass):
    llm_groups_full = select_groups_via_llm(
        token_frequency, "mock-llm-model", vocab_sample_k=0
    )
assert isinstance(llm_groups_full, dict)
assert len(llm_groups_full) == 4
print(f"[OK] 7b select_groups_via_llm (full vocab, k=0): "
      f"{sum(len(v) for v in llm_groups_full.values())} tokens across 4 groups")
